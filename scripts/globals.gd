...
var enemy_count: int:
	get:
		return enemies_container.get_children().filter(func(enemy: Enemy) -> bool: return enemy.state != Enemy.State.DEAD).size()
...
func init() -> void:
	...
	tileMapLayer = get_tree().current_scene.get_node("TileMapLayer")
	player = get_tree().current_scene.get_node("Root/Player")
	hud = get_tree().current_scene.get_node("Hud")
	enemies_container = get_tree().current_scene.get_node("Root/Enemies")
...
func _on_game_state_changed(new_state: GameState, _old_state: GameState) -> void:
	match new_state:
		...
		GameState.PLAYING:
			...
			if not SoundsPlayer.is_playing("main_theme"):
...
func _on_enemy_killed(enemy_type: Enemy.EnemyType) -> void:
	...
	if all_enemies_dead():
		SoundsPlayer.play_sound("all_killed")
...
func set_astar_point_enabled(point: Vector2) -> void:
	main_scene.astar_grid.set_point_solid(Utils.position_to_cell_position(point), false)
	
func set_astar_point_disabled(point: Vector2) -> void:
	main_scene.astar_grid.set_point_solid(Utils.position_to_cell_position(point), true)
